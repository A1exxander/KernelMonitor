	...
	SIZE_T* bytes_read) -> NTSTATUS
{
	...
	return KLI_FN(MmCopyMemory)(buffer, to_read, size, MM_COPY_MEMORY_PHYSICAL, bytes_read);
}
	...
	SIZE_T* bytes_read)
{
	...
	PVOID sp_mapped_memory = KLI_FN(MmMapIoSpaceEx)(to_write, size, PAGE_READWRITE);
	if (!sp_mapped_memory) {
		return STATUS_UNSUCCESSFUL;
	}
	KLI_FN(memcpy)(sp_mapped_memory, buffer, size);
	*bytes_read = size;
	KLI_FN(MmUnmapIoSpace)(sp_mapped_memory, size);
...
namespace pml
{
	...
	static NTSTATUS InitializeMmPfnDatabase()
	{
		...
		const auto p_MmGetVirtualForPhysical = reinterpret_cast<UCHAR*>(((KLI_FN(MmGetVirtualForPhysical))));
	...
	uintptr_t dirbase_from_base_address(void* base)
	{
		...
		auto ranges = KLI_FN(MmGetPhysicalMemoryRanges)();
...
NTSTATUS FixDTB(dtbl gja)
{
	...
	PEPROCESS procc = 0;
	KLI_FN(PsLookupProcessByProcessId)((HANDLE)gja->process_id, &procc);
	if (!procc) {
		return STATUS_UNSUCCESSFUL;
	}
