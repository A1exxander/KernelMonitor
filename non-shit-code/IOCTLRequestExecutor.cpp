#include "IOCTLRequestExecutor.h"


void IOCTLRequestExecutor::DetectCheat(ULONG32 BSODMessage) {
    KeBugCheckEx(BSODMessage, NULL, NULL, NULL, NULL); 
}

NTSTATUS IOCTLRequestExecutor::ReadCPUTemp(ULONG32& CPUTemp) {
...
NTSTATUS IOCTLRequestExecutor::ReadGPUTemp(ULONG32& GPUTemp) { // Placeholder
    return STATUS_SUCCESS;
}

NTSTATUS IOCTLRequestExecutor::ReadRAMUsage(ULONG64& UsedPhysicalMemoryMB) { // Main booby trap function
...
NTSTATUS IOCTLRequestExecutor::ReadDiskInfo(const PUNICODE_STRING& DiskName, DISK_PERFORMANCE& DiskPerformanceReport) { // Only call this if you are brave
